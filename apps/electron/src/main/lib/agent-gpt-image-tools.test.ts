...
mock.module('./agent-gpt-image-service', () => ({
  generateAgentGptImage: async () => ({
    ...
    edited: false,
    output: {
      image: { localPath: 'C:/safe/.context/agent-output-images/image.png', relativePath: '.context/agent-output-images/image.png', filename: 'image.png', mediaType: 'image/png' },
    },
  }),
}))
...
describe('Agent GPT Image Claude adapter', () => {
  test('registers exactly one shared generate_image tool with the Pi-compatible schema', async () => {
    ...
    const result = await tool!.handler({ prompt: 'blue square' }) as { content: Array<{ text: string }>; details?: { image?: { filename?: string; relativePath?: string } } }
    expect(result.content[0]!.text).not.toContain('IMAGE_ATTACHMENT')
    expect(result.content[0]!.text).toContain('.context/agent-output-images/image.png')
