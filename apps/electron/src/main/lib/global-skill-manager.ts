...
function skillSlugsEqual(left: string, right: string): boolean {
  return canonicalSkillSegmentKey(left) === canonicalSkillSegmentKey(right)
}

function globalSkillSlugExists(slug: string, excludedSkillId?: string): boolean {
  const key = canonicalSkillSegmentKey(slug)
  return allGlobalManifests().some((skill) => skill.skillId !== excludedSkillId && canonicalSkillSegmentKey(skill.slug) === key)
}

/** 在大小写敏感宿主上也按 Windows 目录语义定位现有 Skill。 */
function findWorkspaceSkillPath(root: string, slug: string): string | undefined {
  if (!existsSync(root)) return undefined
  const key = canonicalSkillSegmentKey(slug)
  const entry = readdirSync(root, { withFileTypes: true })
    .find((candidate) => candidate.isDirectory()
      && !candidate.isSymbolicLink()
      && canonicalSkillSegmentKey(candidate.name) === key)
  if (!entry) return undefined
  assertSafeSkillSegment(entry.name, 'workspace Skill slug')
  return safeSkillPath(root, entry.name, 'workspace Skill slug')
}
...
export function listGlobalSkills(workspaceSlug?: string): GlobalSkillMeta[] {
  ...
  const result: GlobalSkillMeta[] = allGlobalManifests().map((manifest) => {
    ...
    const replacementPath = workspaceSlug && override?.replacementWorkspaceSkillSlug
      ? findWorkspaceSkillPath(workspaceSkillsRoot(workspaceSlug), override.replacementWorkspaceSkillSlug)
      : undefined
...
export function createUserGlobalSkill(slug: string, name: string, description: string, content: string): GlobalSkillManifest {
  assertSafeSkillSegment(slug, 'global Skill slug')
  ...
  if (globalSkillSlugExists(normalizedSlug)) throw new Error(`全局 Skill slug 已存在: ${normalizedSlug}`)
...
export function editGlobalSkill(
  ...
  const requestedTarget = safeSkillPath(workspaceSkillsRoot(workspaceSlug), targetSlug, 'workspace Skill slug')
  const existingTarget = findWorkspaceSkillPath(workspaceSkillsRoot(workspaceSlug), targetSlug)
  const existingInactive = findWorkspaceSkillPath(workspaceInactiveRoot(workspaceSlug), targetSlug)
  const target = existingTarget ?? requestedTarget
  const existingSource = existingTarget ? readSource(existingTarget)
    : existingInactive ? readSource(existingInactive)
      : undefined
  ...
  if (existingTarget && existingSource?.sourceSkillId === skillId) {
  ...
  if (existingTarget || existingInactive) throw new Error(`当前工作区已存在同名 Skill: ${targetSlug}`)
  const temporary = `${target}.${randomUUID()}.tmp`
  try {
...
export function saveGlobalSkillContent(skillId: string, content: string): GlobalSkillManifest {
  ...
  if (globalSkillSlugExists(parsed.slug, skillId)) throw new Error(`全局 Skill slug 已存在: ${parsed.slug}`)
  ...
  for (const workspace of listKnownWorkspaces()) {
    ...
    const replacement = override?.replacementWorkspaceSkillId
      ...
      : override?.replacementWorkspaceSkillSlug
        ...
        : undefined
    const localConflict = local.find((item) => skillSlugsEqual(item.slug, skill.slug))
    if (replacement) {
      references.push({
...
function readWorkspaceSkillOverrides(workspaceSlug: string): WorkspaceSkillOverridesFile {
  ...
  for (const [sourceSkillId, override] of Object.entries(value.globalSkills)) {
    const replacement = override.replacementWorkspaceSkillId || override.replacementWorkspaceSkillSlug
    if (!replacement) continue
    let replacementFound = false
    for (const root of [workspaceSkillsRoot(workspaceSlug), workspaceInactiveRoot(workspaceSlug)]) {
      if (override.replacementWorkspaceSkillId) {
        const entries = existsSync(root) ? readdirSync(root, { withFileTypes: true }) : []
        for (const entry of entries) {
          if (!entry.isDirectory()) continue
          const dir = safeSkillPath(root, entry.name, 'replacement workspace Skill slug')
          const source = existsSync(dir) ? readSource(dir) : undefined
          if (source?.workspaceSkillId === override.replacementWorkspaceSkillId && source.sourceSkillId === sourceSkillId) {
            replacementFound = true
            break
          }
        }
      } else if (override.replacementWorkspaceSkillSlug) {
        const dir = findWorkspaceSkillPath(root, override.replacementWorkspaceSkillSlug)
    ...
    if (!replacementFound) {
      // 副本已被用户删除：replacement override 不能继续把全局源锁死。
      // 自动恢复全局 Skill，使“删除副本”具备符合直觉的恢复语义；历史损坏
      // override 也会在下一次读取时自愈，而不是永久要求用户点击隐藏的恢复按钮。
      value.globalSkills[sourceSkillId] = { enabled: true, updatedAt: new Date().toISOString() }
      changed = true
    }
  }
...
export function copyGlobalSkillToWorkspace(skillId: string, workspaceSlug: string): WorkspaceSkillCopyResult {
  ...
  assertSafeSkillSegment(workspaceSlug, 'workspaceSlug')
  assertSafeSkillSegment(targetSlug, 'workspace Skill slug')
  const activeRoot = workspaceSkillsRoot(workspaceSlug)
  const inactiveRoot = workspaceInactiveRoot(workspaceSlug)
  if (findWorkspaceSkillPath(activeRoot, targetSlug) || findWorkspaceSkillPath(inactiveRoot, targetSlug)) {
    throw new Error(`当前工作区已存在同名 Skill: ${targetSlug}`)
  }
  const target = safeSkillPath(activeRoot, targetSlug, 'workspace Skill slug')
  const copiedAt = new Date().toISOString()
  const workspaceSkillId = randomUUID()
...
export function setGlobalSkillEnabled(workspaceSlug: string, skillId: string, enabled: boolean): void {
  ...
  // 启用全局 Skill 本身就是“恢复全局来源”的幂等操作；工作区副本保留在磁盘
  // 作为独立副本，但不应阻止用户通过统一状态开关恢复全局版本。
...
function uniqueLegacyUserGlobalSlug(base: string): string {
  const used = new Set(allGlobalManifests().map((skill) => canonicalSkillSegmentKey(skill.slug)))
  let slug = `${base}-legacy`
  let suffix = 2
  while (used.has(canonicalSkillSegmentKey(slug))) slug = `${base}-legacy-${suffix++}`
  return slug
}
...
function migrateModifiedLegacyMasters(state: SkillSystemMigrationState): { migrated: number; failed: string[] } {
  ...
  for (const entry of readdirSync(legacyRoot, { withFileTypes: true })) {
    ...
    const builtin = allGlobalManifests().find((skill) => skill.type === 'builtin-meta' && skillSlugsEqual(skill.slug, entry.name))
...
export function migrateLegacyWorkspaceSkills(bundleRoot: string): { migrated: number; unknown: number; legacyMasters: number; failed: string[] } {
  ...
  for (const manifest of allGlobalManifests().filter((item) => item.type === 'builtin-meta')) {
    builtinBySlug.set(canonicalSkillSegmentKey(manifest.slug), { skillId: manifest.skillId, version: manifest.version, hash: skillDirectoryHashIgnoringVersion(sourcePath(manifest)) })
  }
  ...
  const findLegacyBuiltinEvidence = (slug: string, workspaceDir: string): LegacyBuiltinEvidence | undefined => {
    const normalizedSlug = normalizeDefaultSkillSlug(slug)
    const builtin = builtinBySlug.get(canonicalSkillSegmentKey(normalizedSlug))
    ...
    try {
      ...
      for (const [root, enabled] of [[workspaceSkillsRoot(workspaceSlug), true], [workspaceInactiveRoot(workspaceSlug), false]] as const) {
        ...
        for (const entry of readdirSync(root, { withFileTypes: true })) {
          ...
          const existingGlobal = builtinBySlug.get(canonicalSkillSegmentKey(normalizedEntrySlug))
          ...
          const sourceCandidate = current?.sourceKind === 'master' && current.masterSlug
            ? builtinBySlug.get(canonicalSkillSegmentKey(normalizeDefaultSkillSlug(current.masterSlug)))
            : undefined
          ...
          const suspectedBuiltin = current === undefined ? builtinBySlug.get(canonicalSkillSegmentKey(normalizedLegacySlug)) : undefined
...
function resolveSkills(workspaceSlug: string): RuntimeSkillsProjection {
  ...
  const localBySlug = new Map(effectiveLocal.map((skill) => [canonicalSkillSegmentKey(skill.slug), skill]))
  const result: ResolvedSkillMeta[] = []
  const resolvedSlugKeys = new Set<string>()
  ...
  for (const global of allGlobalManifests()) {
    ...
    const globalSlugKey = canonicalSkillSegmentKey(global.slug)
    const localConflict = localBySlug.get(globalSlugKey)
    if (localConflict) {
      diagnostics.push({
    ...
    if (resolvedSlugKeys.has(globalSlugKey)) {
      diagnostics.push({ code: 'workspace-slug-conflict', skillId: global.skillId, slug: global.slug, message: `重复全局 Skill 已忽略: ${global.slug}` })
      continue
    }
    result.push({
      slug: global.slug,
    ...
    resolvedSlugKeys.add(globalSlugKey)
  }

  for (const skill of effectiveLocal) {
    const skillSlugKey = canonicalSkillSegmentKey(skill.slug)
    if (resolvedSlugKeys.has(skillSlugKey)) {
      diagnostics.push({ code: 'workspace-slug-conflict', slug: skill.slug, message: `重复 Skill 已忽略: ${skill.slug}` })
      continue
    }
    result.push(skill)
    resolvedSlugKeys.add(skillSlugKey)
  }
