...
describe('agent block copy serialization', () => {
  test('操作条位于当前连续选区右上角外部，表格不再渲染旧操作条或空气墙', async () => {
    ...
    expect(source).toContain("selectionGroupStart && 'pt-2'")
    expect(source).toContain("selectionGroupEnd && 'pb-2'")
  ...
  test('streaming 纯文本分支保持不变，展开思考允许外部操作条溢出', async () => {
    ...
    expect(contentBlockSource).toContain("shouldCollapse && !isExpanded ? 'max-h-[5.6em] overflow-hidden' : 'overflow-visible'")
  })
