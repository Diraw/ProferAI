...
function PlanningStat({ label, value }: { label: string; value: number }): React.ReactElement {
  return <span className="inline-flex items-center gap-1 rounded-full bg-muted/65 px-2 py-1 tabular-nums"><span>{label}</span><strong className="font-semibold text-foreground">{value}</strong></span>
}
...
export function PlanningView({ standalone = false }: { standalone?: boolean } = {}): React.ReactElement {
  ...
  const weekEnd = endOfToday(addLocalDays(Date.now(), 7))
  const openTodoCount = todos.filter((todo) => todo.status === 'open').length
  const todayTodoCount = todos.filter((todo) => todo.status === 'open' && todo.dueAt !== undefined && todo.dueAt <= todayEnd).length
  const weekTodoCount = todos.filter((todo) => todo.status === 'open' && todo.dueAt !== undefined && todo.dueAt > todayEnd && todo.dueAt <= weekEnd).length
  ...
  const nextPlannedTodo = todos
    .filter((todo) => todo.status === 'open' && todo.dueAt !== undefined)
  ...
  const openNextPlannedTodo = React.useCallback(() => {
    if (!nextPlannedTodo) return
...
function TodoWorkspace({ standalone = false, teamWorkspaceId, embedded = false }: { standalone?: boolean; teamWorkspaceId?: string; embedded?: boolean } = {}): React.ReactElement {
  ...
  const [quickAssigneeId, setQuickAssigneeId] = React.useState('__none__')
  ...
  React.useEffect(() => {
    ...
    void window.electronAPI.team.getMembers(teamWorkspaceId).then((items) => {
      // 团队成员接口的稳定字段是 userId（不是 id）；此前直接断言导致 SelectItem
      // 的 key/value 都是 undefined，从而触发 React key 警告并使负责人无法正确选择。
      const members = (items as Array<{ userId?: unknown; displayName?: unknown }>).flatMap((item) =>
        typeof item.userId === 'string' && item.userId
          ? [{ id: item.userId, displayName: typeof item.displayName === 'string' && item.displayName.trim() ? item.displayName : '未命名成员' }]
          : [],
      )
  ...
  const updateTodo = React.useCallback(async (
    ...
    try {
      const updated = await window.electronAPI.updateTodo({ id, ...input, workspaceId: teamWorkspaceId })
      if (updated) {
  ...
  const createTodo = async (): Promise<void> => {
    ...
    try {
      const todo = await window.electronAPI.createTodo({
        ...
        assigneeId: quickAssigneeId === '__none__' ? undefined : quickAssigneeId,
        // 数据层会以计划完成时间创建默认提醒。
      })
  ...
  const createTodoGroup = React.useCallback(async (name: string): Promise<PlanningGroup | undefined> => {
    try {
      const group = await window.electronAPI.createPlanningGroup({ scope: 'todo', name, workspaceId: teamWorkspaceId })
  ...
  const renameTodoGroup = React.useCallback(async (group: PlanningGroup, name: string): Promise<PlanningGroup | undefined> => {
    try {
      const updated = await window.electronAPI.updatePlanningGroup({ id: group.id, scope: 'todo', name, workspaceId: teamWorkspaceId })
  ...
  const deleteTodoGroup = React.useCallback(async (group: PlanningGroup): Promise<boolean> => {
    try {
      const deleted = await window.electronAPI.deletePlanningGroup('todo', group.id, teamWorkspaceId)
  ...
  const deleteTodo = async (): Promise<void> => {
    ...
    if (!todo) return
    try {
      await window.electronAPI.deleteTodo(todo.id, teamWorkspaceId)
      ...
      <div className="grid h-full min-h-0 w-full grid-cols-[minmax(10.5rem,13.5rem)_minmax(0,1fr)] overflow-hidden">
        <aside className="flex min-h-0 flex-col overflow-y-auto border-r border-surface-border/45 bg-surface-sunken/35 p-3 scrollbar-thin">
          ...
          <nav className="space-y-1" aria-label="Todo 清单">
            {navItems.map((item) => <button key={item.id} type="button" onClick={() => { setView(item.id); setSelectedId(null) }} className={cn('relative flex h-10 w-full items-center gap-2 rounded-xl px-2.5 text-sm transition-colors', view === item.id ? 'bg-surface-raised font-semibold text-foreground shadow-sm ring-1 ring-surface-border/35 before:absolute before:inset-y-2 before:left-0 before:w-0.5 before:rounded-full before:bg-primary' : 'text-muted-foreground hover:bg-surface-selected/70 hover:text-foreground')}><span className="text-muted-foreground">{item.icon}</span><span className="flex-1 text-left">{item.label}</span>{item.count !== undefined && <span className={cn('rounded-full px-1.5 py-0.5 text-[11px] tabular-nums', view === item.id ? 'bg-primary/10 text-primary' : 'bg-muted/70 text-muted-foreground')}>{item.count}</span>}</button>)}
          </nav>
          <div className="mt-7"><div className="flex items-center justify-between px-2 pb-2"><span className="text-xs font-semibold uppercase tracking-[0.12em] text-muted-foreground">Todo 分组</span><PlanningGroupManager scope="todo" groups={groups} itemLabel="Todo" getUsageCount={(groupId) => todoGroupUsageCounts.get(groupId) ?? 0} hasAssociatedItems={(groupId) => todoGroupHasAssociatedItems.has(groupId)} showDeletionCount={false} onCreate={createTodoGroup} onRename={renameTodoGroup} onDelete={deleteTodoGroup} onCreated={(group) => { setView(`group:${group.id}`); setSelectedId(null) }} trigger={<button type="button" className="min-h-10 px-1 text-xs font-medium text-muted-foreground transition-colors hover:text-foreground">管理</button>} /></div><div className="space-y-1">{groups.map((group) => { const id = `group:${group.id}` as TodoListView; return <button key={group.id} type="button" onClick={() => { setView(id); setSelectedId(null) }} className={cn('flex h-10 w-full items-center gap-2 rounded-xl px-2.5 text-sm transition-colors', view === id ? 'bg-surface-raised font-semibold text-foreground shadow-sm ring-1 ring-surface-border/35' : 'text-muted-foreground hover:bg-surface-selected/70 hover:text-foreground')}><span className="size-2 rounded-full" style={{ backgroundColor: group.color ?? 'currentColor' }} /><span className="flex-1 truncate text-left">{group.name}</span><span className="rounded-full bg-muted/70 px-1.5 py-0.5 text-[11px] tabular-nums text-muted-foreground">{todoGroupUsageCounts.get(group.id) ?? 0}</span></button> })}</div></div>
        </aside>

        <div className="flex min-h-0 min-w-0 flex-col overflow-hidden">
          <div className="flex items-center justify-between border-b border-surface-border/45 px-5 py-4"><div><h2 className="text-base font-semibold">{viewTitle}</h2><p className="mt-0.5 text-xs text-muted-foreground">专注处理当前最重要的事项</p></div>{view !== 'completed' && <span className="rounded-full bg-muted px-2 py-1 text-xs tabular-nums text-muted-foreground">{visibleTodos.length} 项</span>}</div>
          <div className="min-h-0 flex-1 overflow-y-auto">{visibleTodos.length ? visibleTodos.map((todo) => <TodoListItem key={todo.id} todo={todo} selected={selectedId === todo.id} todayEnd={todayEnd} onSelect={() => setSelectedId(todo.id)} onToggle={() => void completeTodo(todo)} onDelete={() => setTodoPendingDeletion(todo)} />) : <TodoEmptyState onCreate={openCreateDialog} />}</div>
        </div>
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="top-[34%] max-w-xl gap-0 p-3" hideClose>
          ...
          <form onSubmit={(event) => { event.preventDefault(); void createTodo() }} className="overflow-hidden rounded-none border border-surface-border/60 bg-surface-raised focus-within:ring-1 focus-within:ring-focus/40">
            ...
            <div className="flex flex-wrap items-center gap-x-1 gap-y-1 border-t border-surface-border/60 bg-surface-sunken/30 px-1.5 py-1.5">
              ...
              {teamWorkspaceId && <Select value={quickAssigneeId} onValueChange={setQuickAssigneeId}><SelectTrigger className="h-9 w-auto max-w-44 border-0 bg-transparent px-2 text-sm shadow-none hover:bg-muted"><SelectValue placeholder="负责人" /></SelectTrigger><SelectContent><SelectItem value="__none__">未指派</SelectItem>{teamMembers.map((member) => <SelectItem key={`quick-assignee-${member.id}`} value={member.id}>{member.displayName}</SelectItem>)}</SelectContent></Select>}
...
function TodoEmptyState({ onCreate }: { onCreate: () => void }): React.ReactElement {
  return <div className="flex h-full min-h-56 items-center justify-center px-8 text-center">
    <div className="max-w-sm"><div className="mx-auto flex size-11 items-center justify-center rounded-2xl bg-primary/10 text-primary"><ListTodo size={21} /></div><h3 className="mt-4 text-sm font-semibold text-foreground">从一个清晰的待办开始</h3><p className="mt-1.5 text-sm leading-6 text-muted-foreground">把要做的事记下来，规划中心会帮你安排后续节奏。</p><button type="button" onClick={onCreate} className="mt-4 inline-flex h-9 items-center gap-1.5 rounded-xl bg-primary px-3 text-sm font-medium text-primary-foreground shadow-sm transition-colors hover:bg-primary/90"><Plus size={15} />新建 Todo</button></div>
  </div>
}
...
function TodoListItem({
  ...
  return (
    <div
      ...
      className={cn('group flex min-h-[72px] w-full items-center gap-3 border-b border-border/40 px-5 py-2.5 text-left transition-colors focus:outline-none', selected ? 'bg-primary/8 shadow-[inset_3px_0_0_hsl(var(--primary))]' : 'hover:bg-muted/40 focus-visible:bg-muted/40')}
    >
