...
export function installElectronApiStub(): void {
  ...
  const stub: Record<string, unknown> = {
    ...
    // 平板不能访问桌面本地 Office viewer；显式返回失败，让主进程 preview session 超时/报错，绝不伪造 ready。
    getAgentFilePreviewRevision: () => Promise.resolve(null),
    reportAgentFilePreview: () => Promise.resolve(false),
    reportAgentFilePreviewInspection: () => Promise.resolve(false),
    ...
    getAgentRuntimeCapabilities: (runtime: 'claude' | 'pi') => Promise.resolve({
      runtime,
      available: true,
